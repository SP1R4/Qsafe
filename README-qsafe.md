...
- **Encrypted credential store** — `secrets set/get/list/rm`: a password-manager-style key-value store layered on a vault volume, inheriting its deniability. Values move only in memory (never a temp file): `printf '%s' "$TOKEN" | qsafe secrets set github.token`, `qsafe secrets get github.token`
